...
import fr.ab.MovieAssistant.DTO.Request.QueryResultDTO;
...
public class PizzaService {
    ...
    public WebhookReponseDTO getPizza(QueryRequestDTO queryRequestDTO) {
        ...
        if (queryRequestDTO.getQueryResult().getParameters().getBase() == null && queryRequestDTO.getQueryResult().getParameters().getReponse() == null) {
        ...
        if (queryRequestDTO.getQueryResult().getParameters().getReponse() != null) {
            return getDessertByReponse(queryRequestDTO.getQueryResult());
        }
    ...
    private WebhookReponseDTO getChoice(ContextDTO contextDTO) {
        ...
        simpleResponseDTO.setTextToSpeech("Un supplément ? avec votre pizza " + contextDTO.getParameters().get("OPTION") + " ?");
    ...
    private WebhookReponseDTO getDessertByReponse(QueryResultDTO reponse) {
        ...
        if (reponse.getParameters().getReponse().equals("oui")) {
        ...
        //CALCUL
        String prix = "";
        List<ContextDTO> context = reponse.getOutputContexts();
        for (ContextDTO contextDTO : context) {
            if (contextDTO.getParameters().containsKey("pizza")) {
                prix = contextDTO.getParameters().get("pizza").toString();
            }
        }
        ...
        SimpleResponsesDTO simpleResponsesDTO = new SimpleResponsesDTO();
        SimpleResponseDTO simpleResponseDTO = new SimpleResponseDTO();
        simpleResponseDTO.setTextToSpeech("Très bon choix, nous préparons ta commande, ça te fera" + prix + "€");
        simpleResponsesDTO.setSimpleResponses(List.of(simpleResponseDTO));
        messageSimpleResponse.setSimpleResponses(simpleResponsesDTO);
        messageDTOList.add(messageSimpleResponse);
        return webhookReponseDTO;
